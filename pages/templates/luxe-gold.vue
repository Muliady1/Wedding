...
import { ZoomIn, Copy, Check, Heart, MessageSquare, Clock, MapPin, ChevronDown, Menu, X, Crown, Sparkles, Gem, Diamond } from 'lucide-vue-next'
...
<template>
  <div class="min-h-screen bg-[#1A1A1A] text-[#D4AF37] font-sans">
    ...
    <!-- Navigation -->
    <nav class="fixed top-0 left-0 right-0 z-50 bg-[#1A1A1A]/95 backdrop-blur-md border-b border-[#D4AF37]/10">
      <div class="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
        <div class="text-lg font-serif tracking-wider flex items-center gap-2">
          <Crown class="w-5 h-5 text-[#D4AF37]" />
          {{ props.groom }} & {{ props.bride }}
        </div>
        <button @click="showMenu = !showMenu" class="p-2 hover:bg-[#D4AF37]/10 rounded-lg transition-colors">
          <Menu v-if="!showMenu" class="w-6 h-6" />
          <X v-else class="w-6 h-6" />
        </button>
      </div>
      <div v-if="showMenu" class="px-6 pb-4 flex flex-col gap-3 animate-slideDown">
        <button @click="scrollToSection('home')" class="text-left py-2 hover:text-[#D4AF37] transition-colors">Home</button>
        <button @click="scrollToSection('couple')" class="text-left py-2 hover:text-[#D4AF37] transition-colors">Couple</button>
        <button @click="scrollToSection('story')" class="text-left py-2 hover:text-[#D4AF37] transition-colors">Story</button>
        <button @click="scrollToSection('gallery')" class="text-left py-2 hover:text-[#D4AF37] transition-colors">Gallery</button>
        <button @click="scrollToSection('event')" class="text-left py-2 hover:text-[#D4AF37] transition-colors">Event</button>
        <button @click="scrollToSection('gift')" class="text-left py-2 hover:text-[#D4AF37] transition-colors">Gift</button>
        <button @click="scrollToSection('rsvp')" class="text-left py-2 hover:text-[#D4AF37] transition-colors">RSVP</button>
      </div>
    </nav>
