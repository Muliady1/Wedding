...
const stories = rusticChicData.stories
const galleryImages = rusticChicData.galleryImages
const accounts = rusticChicData.accounts
const qrCodes = rusticChicData.qrCodes
const eventLocations = rusticChicData.eventLocations
...
<template>
  <div class="min-h-screen bg-[#F5F0E6] text-[#5D4E37] font-sans">
    <!-- Background Textures -->
    ...
    <section id="home" class="min-h-screen flex items-center justify-center relative overflow-hidden">
      ...
      <div class="relative z-10 text-center px-6">
        <div class="animate-fadeInUp">
          <p class="text-sm tracking-[0.4em] uppercase text-[#8B7355]/70 mb-4">The Wedding Of</p>
        </div>
        <div class="animate-fadeInUp" style="animation-delay: 0.2s;">
          <Mountain class="w-16 h-16 mx-auto mb-6 text-[#8B7355]" />
          <h1 class="text-5xl md:text-7xl font-serif mb-6">{{ props.groom }} <span class="text-[#8B7355]/40">&</span> {{
            props.bride }}</h1>
        </div>
        <div class="animate-fadeInUp" style="animation-delay: 0.4s;">
          <p class="text-lg text-[#5D4E37]/70 mb-8">{{ formattedDate }}</p>
        </div>
        <div class="animate-fadeIn" style="animation-delay: 0.6s;">
          <div class="flex flex-col items-center gap-2">
            <p class="text-sm text-[#8B7355]/50">Kepada Yth.</p>
            <p class="text-xl font-medium text-[#5D4E37]">{{ guestName }}</p>
          </div>
        </div>
      </div>
      <div class="absolute bottom-10 left-1/2 -translate-x-1/2 animate-bounce">
        <button @click="scrollToSection('couple')"
          class="p-3 rounded-full border border-[#8B7355]/30 hover:border-[#8B7355]/60 transition-colors">
          <ChevronDown class="w-6 h-6 text-[#8B7355]" />
        </button>
      </div>
    </section>
    ...
    <section id="couple" class="py-24 px-6 relative">
      <div class="max-w-4xl mx-auto">
        <div
          ...
          <p class="text-sm tracking-[0.3em] uppercase text-[#8B7355]/70 mb-4">Mempelai</p>
          <h2 class="text-4xl font-serif text-[#5D4E37]">Couple</h2>
          <div class="flex justify-center mt-4 gap-2">
            <Leaf class="w-4 h-4 text-[#8B7355]/50" />
            <Wheat class="w-4 h-4 text-[#8B7355]/50" />
          </div>
        </div>
        <div class="grid md:grid-cols-2 gap-16">
          <div
            ...
            <div class="aspect-[3/4] bg-[#D4C8B8] mb-6 overflow-hidden rounded-lg">
              <NuxtImg src="https://picsum.photos/seed/groom6/600/800" alt="Groom" class="w-full h-full object-cover" />
            </div>
            <h3 class="text-2xl font-serif mb-2 text-[#5D4E37]">{{ props.groom }}</h3>
            <p class="text-[#8B7355]/70">Putra dari Bpk. Heru & Ibu Siti</p>
          </div>
          <div
            ...
            <div class="aspect-[3/4] bg-[#D4C8B8] mb-6 overflow-hidden rounded-lg">
              <NuxtImg src="https://picsum.photos/seed/bride6/600/800" alt="Bride" class="w-full h-full object-cover" />
            </div>
            <h3 class="text-2xl font-serif mb-2 text-[#5D4E37]">{{ props.bride }}</h3>
            <p class="text-[#8B7355]/70">Putri dari Bpk. Ahmad & Ibu Aminah</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="py-20 px-6 bg-[#5D4E37] text-[#F5F0E6]">
      <div class="max-w-3xl mx-auto">
        <div
          ...
          <p class="text-sm tracking-[0.3em] uppercase text-[#D4C8B8] mb-4">Hitung Mundur</p>
          <h2 class="text-3xl font-serif">Menuju Hari Bahagia</h2>
        </div>
        <div class="grid grid-cols-4 gap-4">
          <div v-for="(item, key) in timeLeft" :key="key"
            :class="['text-center p-6 bg-[#F5F0E6]/5 border border-[#8B7355]/30 transition-all duration-500', isVisible('couple') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10']"
            ...
            <div class="text-xs tracking-[0.2em] uppercase text-[#D4C8B8]">{{ key }}</div>
          </div>
        </div>
      </div>
    </section>
    ...
    <section id="story" class="py-24 px-6 relative">
      <div class="max-w-4xl mx-auto relative">
        <div
          ...
          <p class="text-sm tracking-[0.3em] uppercase text-[#8B7355]/70 mb-4">Kisah Cinta</p>
          <h2 class="text-4xl font-serif text-[#5D4E37]">Our Story</h2>
        </div>
        <div class="relative">
          <div class="absolute left-1/2 -translate-x-1/2 w-px h-full bg-[#8B7355]/20"></div>
          <div class="space-y-16">
            <div v-for="(story, index) in coupleStories" :key="index"
              ...
              <div class="max-w-sm w-1/2">
                <div
                  class="w-12 h-12 bg-[#8B7355] text-white rounded-full flex items-center justify-center text-sm font-serif mb-4">
                  {{ story.number }}</div>
                <p class="text-sm text-[#8B7355]/70 mb-2">{{ story.year }}</p>
                <h3 class="text-xl font-serif mb-2 text-[#5D4E37]">{{ story.title }}</h3>
                <p class="text-[#5D4E37]/70 leading-relaxed">{{ story.desc }}</p>
              </div>
            </div>
          </div>
        </div>
    ...
    <section id="event" class="py-24 px-6 relative">
      <div class="max-w-4xl mx-auto">
        <div
          ...
          <p class="text-sm tracking-[0.3em] uppercase text-[#8B7355]/70 mb-4">Jadwal</p>
          <h2 class="text-4xl font-serif text-[#5D4E37]">Wedding Event</h2>
        </div>
        <div class="grid md:grid-cols-2 gap-8">
          <div
            :class="['p-8 border border-[#8B7355]/20 hover:border-[#8B7355]/50 transition-all duration-300 bg-white/50 backdrop-blur-sm rounded-lg', isVisible('event') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10']"
            style="transition-delay: 0.1s;">
            <div class="flex items-center gap-3 mb-4">
              <Home class="w-5 h-5 text-[#8B7355]" />
              <h3 class="text-xl font-serif text-[#5D4E37]">{{ eventLocations.akad.title }}</h3>
            </div>
            <div class="space-y-3 text-[#5D4E37]/70">
          ...
          <div
            ...
            <div class="flex items-center gap-3 mb-4">
              <Leaf class="w-5 h-5 text-[#8B7355]" />
              <h3 class="text-xl font-serif text-[#5D4E37]">{{ eventLocations.resepsi.title }}</h3>
            </div>
            <div class="space-y-3 text-[#5D4E37]/70">
              <div class="flex items-center gap-2">
        ...
        <div
          ...
          <button @click="addToCalendar"
            class="px-8 py-3 bg-[#5D4E37] text-white hover:bg-[#8B7355] transition-colors tracking-widest text-sm rounded">
            ADD TO CALENDAR
          </button>
        </div>
      </div>
    ...
    <section id="gift" class="py-24 px-6 bg-[#D4C8B8]/30">
      <div class="max-w-4xl mx-auto">
        <div
          ...
          <p class="text-sm tracking-[0.3em] uppercase text-[#8B7355]/70 mb-4">Hadiah</p>
          <h2 class="text-4xl font-serif text-[#5D4E37]">Wedding Gift</h2>
        </div>
        <div class="flex justify-center mb-8">
          <div class="flex gap-2">
            <button @click="activeTab = 'bank'"
              :class="['px-6 py-2 text-sm tracking-widest transition-colors rounded', activeTab === 'bank' ? 'bg-[#5D4E37] text-white' : 'bg-white border border-[#8B7355]/30']">BANK
              TRANSFER</button>
            <button @click="activeTab = 'digital'"
              :class="['px-6 py-2 text-sm tracking-widest transition-colors rounded', activeTab === 'digital' ? 'bg-[#5D4E37] text-white' : 'bg-white border border-[#8B7355]/30']">DIGITAL
              WALLET</button>
          </div>
        </div>
        <div v-if="activeTab === 'bank'" class="grid md:grid-cols-2 gap-6">
          <div v-for="(account, index) in accounts" :key="account.bank"
            :class="['p-6 bg-white border border-[#8B7355]/20 transition-all duration-500 rounded', isVisible('gift') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10']"
            :style="{ transitionDelay: index * 0.1 + 's' }">
            <div class="flex items-center justify-between mb-4">
              <div class="w-10 h-10 bg-[#8B7355] text-white flex items-center justify-center font-serif">{{ account.logo
                }}</div>
              <span class="font-serif">{{ account.bank }}</span>
            </div>
            <p class="text-lg mb-1">{{ account.number }}</p>
            <p class="text-[#8B7355]/70 text-sm mb-4">{{ account.owner }}</p>
            <button @click="copyToClipboard(account.number, index)"
              class="w-full py-2 border border-[#8B7355]/30 hover:border-[#8B7355]/60 transition-colors text-sm flex items-center justify-center gap-2 rounded">
        ...
        <div v-else class="grid md:grid-cols-2 gap-6">
          <div v-for="(qr, index) in qrCodes" :key="qr.bank"
            :class="['p-6 bg-white border border-[#8B7355]/20 text-center transition-all duration-500 rounded', isVisible('gift') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10']"
            :style="{ transitionDelay: index * 0.1 + 's' }">
            <div
              class="w-16 h-16 bg-[#8B7355] text-white flex items-center justify-center text-2xl font-serif mx-auto mb-4">
              {{ qr.logo }}</div>
            <p class="text-lg font-medium">{{ qr.bank }}</p>
            <p class="text-[#8B7355]/70 text-sm mb-4">{{ qr.number }}</p>
    ...
    <section id="rsvp" class="py-24 px-6">
      <div class="max-w-2xl mx-auto">
        <div
          ...
          <p class="text-sm tracking-[0.3em] uppercase text-[#8B7355]/70 mb-4">Konfirmasi</p>
          <h2 class="text-4xl font-serif text-[#5D4E37]">RSVP</h2>
          <p class="text-[#5D4E37]/70 mt-4">Mohon konfirmasi kehadiran Anda sebelum 1 Juni 2026</p>
        </div>
        <div
          :class="['transition-all duration-700', isVisible('rsvp') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10']">
          <form class="space-y-6" @submit.prevent>
            <div>
              <label class="block text-sm text-[#8B7355]/70 mb-2">Nama Tamu<input type="text" :value="guestName"
                  class="w-full p-4 border border-[#8B7355]/30 focus:border-[#8B7355]/60 outline-none transition-colors bg-white/50 rounded" /></label>
            </div>
            <div>
              <label class="block text-sm text-[#8B7355]/70 mb-2">Jumlah Tamu<select
                  class="w-full p-4 border border-[#8B7355]/30 focus:border-[#8B7355]/60 outline-none transition-colors bg-white/50 rounded">
            ...
            <div>
              <p class="block text-sm text-[#8B7355]/70 mb-2">Konfirmasi Kehadiran</p>
              <div class="grid grid-cols-3 gap-4">
                <button type="button"
                  class="p-4 border border-[#8B7355]/30 hover:border-[#8B7355]/60 transition-colors text-center rounded">
                  <Heart class="w-6 h-6 mx-auto mb-2 text-[#8B7355]" />
                  <span class="text-sm">Hadir</span>
                </button>
                <button type="button"
                  class="p-4 border border-[#8B7355]/30 hover:border-[#8B7355]/60 transition-colors text-center rounded">
                  <MessageSquare class="w-6 h-6 mx-auto mb-2 text-[#8B7355]" />
                  <span class="text-sm">Ragu</span>
                </button>
                <button type="button"
                  class="p-4 border border-[#8B7355]/30 hover:border-[#8B7355]/60 transition-colors text-center rounded">
                  <X class="w-6 h-6 mx-auto mb-2 text-[#8B7355]" />
                  <span class="text-sm">Tidak Hadir</span>
                </button>
              </div>
            </div>
            <button type="submit"
              class="w-full py-4 bg-[#5D4E37] text-white hover:bg-[#8B7355] transition-colors tracking-widest text-sm rounded">
              KIRIM RSVP
            </button>
          </form>
        </div>
    ...
    <!-- Footer -->
    <footer class="py-12 px-6 bg-[#5D4E37] text-[#F5F0E6] text-center">
      <p class="text-lg font-serif tracking-widest mb-4">{{ props.groom }} & {{ props.bride }}</p>
      <p class="text-[#D4C8B8] text-sm">Terima kasih atas doa dan restunya</p>
    </footer>
    ...
    <div v-if="showLightbox"
      ...
      <button @click="closeLightbox" class="absolute top-6 right-6 p-2 text-white hover:text-[#8B7355]">
        <X class="w-8 h-8" />
      </button>
    </div>
  </div>
